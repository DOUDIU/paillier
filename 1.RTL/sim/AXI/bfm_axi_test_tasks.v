        ...
        for (idy=0; idy<depth; idy=idy+bsize) begin
            ...
            for (idz=0; idz<bsize; idz=idz+1) begin
                if (dataWB[idz]!=dataRB[idz]) begin
                    error = error + 1;
                    $display("%4d %m Error A:0x%x D:0x%x, but 0x%x expected", $time, addr+idz, dataRB[idz], dataWB[idz]);
                end
                `ifdef DEBUG
    ...
    begin
        ...
        for (idy=0; idy<depth; idy=idy+bsize) begin
            ...
            for (idx=0; idx<bsize; idx=idx+1) begin
                if ((idy+idx+1)!=dataRB[idx]) begin
                    error = error + 1;
                    $display("%m Error A:0x%x D:0x%x, but 0x%x expected", $time, addr+idz, dataRB[idz], idy+idx+1);
                end
                `ifdef DEBUG
                else $display("%4d %m OK A:0x%x D:0x%x", $time, addr+idz, dataRB[idz]);
                `endif
        ...
        if (error==0) $display("%4d %m test_raw_all from 0x%08x to 0x%08x %03d-size %03d-leng OK", $time, saddr, saddr+depth-1, bsize, bleng);
    end
    endtask
    //----------------------------------------------------------------
    // Revision History
